...
文件结构
+ 流式文件：无结构、文本、二进制(Linux都是流)
+ 结构文件：excel等(只是逻辑上的结构)
...
+ 口令保护(比如输入密码)
+ 加密保护(比如异或算法加密)
+ 访问控制(ACL, Access-Control List)，因为在索引节点(FCB) 有权限记录

文件系统层次(从上到下)：
+ 用户接口： 系统调用接口
+ 文件目录系统 ： 找FCB
+ 存取控制模块 ：权限
+ 逻辑文件系统与文件信息缓冲区 ： 文件记录号到逻辑地址
+ 物理文件系统：转为实际物理地址
+ 辅助分配/设备管理
...
+ 类似内存分配
+ 磁盘块一般和内存块一样大，方便页面交换
+ 连续分配：分配连续的文件块。顺序访问速度快
+ 链接分配：指针指到下一个磁盘块(隐式链接)。有FAT表(即文件分配表，来知道下一个磁盘块，显示链接)。
+ 索引分配：有个索引表来知道哪块在哪里。

磁盘
+ 磁道
+ 扇区(每个扇区等大的,盘的内测数据密度大)
+ 磁头
+ 盘面、柱面
+ 磁盘块位置 = 柱面号+盘面号+扇区号
...
### I/O管理
Input/Output，即外部设备，磁盘、网卡、键盘、显示器等。
<br> Unix把外部设备也抽象为文件。
...
+ 一个IO控制器可能对应多个设备
+ 这3种寄存器可能有多个
+ `内存映像IO`：内存中有相应地址。`寄存器独立编址`：不放内存了。

> IO控制方式
+ 程序直接控制：轮询。CPU干预频率高。一次一个Byte单位。实现简单，软件实现。
+ 中断驱动：IO进程阻塞，CPU空出来做别的。弄好了发中断信号就绪，之后可能CPU恢复进程。也是一次一个Byte。中断太多也会干扰CPU。
+ DMA方式：Direct Memory Access。只有开始和结束时需要CPU干预。传送单位是连续的“块”，不连续不好办。`设备直接交互内存。`
+ 通道控制方式：识别一系列的通道指令（比CPU指令少），代理了CPU。CPU只需要告诉他要做一系列的动作。改进了DMA，离散的块也可以。

> IO软件层次
+ 用户层软件：库函数，比如print
+ 设备独立性软件：设备保护、权限；差错处理；设备分配回收；数据缓冲区管理；建立逻辑设备名和物理设备名的映射(LUT)，选择合适驱动；
+ 设备驱动程序
+ 中断处理程序
+ 硬件
...
> 假脱机技术SPOOLing。用软件的方式模拟脱机技术。改造独占式设备为共享式设备，比如打印机，其实就是排队使用。
