...
域名
+ 域名: n级域名...三级域名.二级域名.顶级域名(比如com).(根)
+ 国家顶级域名(cn, us, uk)，通用顶级域名(com,net,org,gov,int,aero,musemum,io)，基础结构域名(只有arpa)
+ 二级域名:类别域名(ac,com,edn,gov,mil,net,org)，行政区(bj,js)


当一个主机发出DNS查询请求时，先查`本地域名服务器`， 再查`根域名服务器`(里面知道顶级域名服务器是哪些)， 再查`顶级域名服务器`,再查权`限域名服务器`(比如一个区).
...
## 电子邮件
+ SMTP：简单邮件传输协议，SMTP客户<-->STMP服务端，服务端也可以作为客户端
+ POP3：邮局协议：下载读取
+ IMAP：网际报文存取协议。用户需要打开邮件时，才下载部分。

## HTTP协议
+ WWW万维网(World Wide Web)
+ 统一资源定位符URL，不区分大小写： <协议>://<主机>:<端口>/<路径>
+ HTML超文本标记语言
+ HTTP超文本传输协议：规定客户端如何请求服务端文档，已经如何传输。
+ 基于TCP
...
HTTP特点
+ 无状态的。可以额外加Cookie、Session来做一下状态
+ TCP作为传输层。但HTTP本身无连接
+ 持久连接(Keep Alive)包括非流水线的，流水线的。即复用TCP连接
+ 非持久连接Close